...
def test_event_service_on(event_service:EventService):
    ...
    event_service.on(test_event_name,callback_test_1)
    # 测试注册是否成功
    assert test_event_name in event_service._hooks , "注册未成功" 
    # 测试callback是否正确
    assert callback_test_1 is event_service._hooks[test_event_name][0] , "callback不同"
    # 测试第二次添加是否成功
    event_service.on(test_event_name,callback_test_2)
    assert callback_test_2 is event_service._hooks[test_event_name][1] , "callback不同"

def test_on_with_non_callable_raises_type_error(event_service:EventService):
    """
    测试场景：错误传入参数类型是否能够正常的报错
    """
    with pytest.raises(TypeError):
        event_service.on("test_event", 123)  # 123 不是可调用对象
...
def test_on_different_events_are_isolated(event_service:EventService):
    ...
    def callback_b():
        print("b")
        return "b"
    event_service.on(event_a,callback_a)
    event_service.on(event_a,callback_b)
    assert event_service._hooks[event_a][0] is callback_a , "导入不正确"
    assert event_service._hooks[event_a][1] is callback_b , "导入不正确"
